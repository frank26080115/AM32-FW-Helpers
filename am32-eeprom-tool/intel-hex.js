 ...
 * https://github.com/NordicSemiconductor/nrf-intel-hex/blob/main/intel-hex.js
 */
 ...
 * Optional newline                                      (?:\r\n|\r|\n|)
 */
const intelhex_hexLineRegexp = /:([0-9A-Fa-f]{8,})([0-9A-Fa-f]{2})(?:\r\n|\r|\n|)/g;


// Takes a Uint8Array as input,
// Returns an integer in the 0-255 range.
function intelhex_checksum(bytes) {
    return (-bytes.reduce((sum, v)=>sum + v, 0)) & 0xFF;
}

// Takes two Uint8Arrays as input,
// Returns an integer in the 0-255 range.
function intelhex_checksumTwo(array1, array2) {
...
function intelhex_hexpad(number) {
...
class MemoryMap {
    ...
    static fromHex(hexText, maxBlockSize = Infinity) {
        ...
        intelhex_hexLineRegexp.lastIndex = 0; // Reset the regexp, if not it would skip content when called twice

        while ((matchResult = intelhex_hexLineRegexp.exec(hexText)) !== null) {
            recordCount++;
            ...
            lastCharacterParsed = intelhex_hexLineRegexp.lastIndex;
            ...
            const cs = intelhex_checksum(recordBytes);
            ...
            if (recordType === 0) {
                ...
                case 5: // Start Linear Address Record
                    ...
                    break;
                default:
                    throw new Error('Invalid record type 0x' + intelhex_hexpad(recordType) + ' at record ' + recordCount + ' (should be between 0x00 and 0x05)');
                }
            }
        }
    ...
    /**
     * Returns the entire memory map as a Uint8Array
     *
     * @return {Uint8Array} the entire buffer
     */
    asUint8Array(padByte=0xFF) {
        const keys = Array.from(this.keys());
        var minAddr = -1;
        var maxAddr = 0;
        var arr = null;
        ...
        var total_length = maxAddr - minAddr;
        arr = new Uint8Array(total_length);
        arr.fill(padByte);
        for (let i=0,l=keys.length; i<l; i++)
        {
            const blockAddr = keys[i];
    ...
    asHexString(lineSize = 16) {
        ...
        for (let i=0,l=sortedKeys.length; i<l; i++) {
            ...
            if (blockAddr > (highAddress + 0xFFFF)) {
                ...
                records.push(
                    ':' +
                    Array.prototype.map.call(offsetRecord, intelhex_hexpad).join('') +
                    intelhex_hexpad(intelhex_checksum(offsetRecord))
                );
            }
            ...
            while (highAddress + lowAddress < blockEnd) {
                ...
                while (lowAddress < 0x10000 && recordSize) {
                    ...
                    if (recordSize) {
                        ...
                        records.push(
                            ':' +
                            Array.prototype.map.call(recordHeader, intelhex_hexpad).join('') +
                            Array.prototype.map.call(subBlock, intelhex_hexpad).join('') +
                            intelhex_hexpad(intelhex_checksumTwo(recordHeader, subBlock))
                        );
